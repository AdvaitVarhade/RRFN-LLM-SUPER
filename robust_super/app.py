...
def run_interactive_simulation(
    ...
    def _execute(device: str):
        ...
        if progress_bar: progress_bar.progress(65, text="Stage 4/6: Reliability-Weighted Pareto Catalog Partitioning...")
        t0 = time.time()
        ...
        if progress_bar: progress_bar.progress(80, text="Stage 5/6: Decoupled Dual Training with Risk-Consistent Loss...")
        t0 = time.time()
...
def run_robustness_sweep_fast(clean_split: DataSplit, movies_df: pd.DataFrame, attack_type: str = "bandwagon", progress_bar=None):
    ...
    for idx, rho in enumerate(noise_levels):
        if progress_bar:
            progress_bar.progress(int((idx + 1) / len(noise_levels) * 100), text=f"Evaluating Attack Budget rho = {rho:.2f}...")
...
<div style="background: linear-gradient(90deg, #1e293b, #0f172a); padding: 18px 24px; border-radius: 12px; margin-bottom: 16px; border-left: 6px solid #3b82f6; box-shadow: 0 4px 14px rgba(0,0,0,0.3);">
    <h2 style="color: #ffffff; margin: 0; font-size: 26px; font-weight: 700; letter-spacing: -0.5px;">RRFN-LLM-SUPER: Robust Recommendation Platform</h2>
...
device_choice = "cpu"  # Streamlit uses CPU for live simulation
...
attack_type = st.sidebar.selectbox(
    ...
    format_func=lambda x: {
        "bandwagon": "Bandwagon Push (5-Star on Target Tail + Head Fillers)",
        "nuke_bomb": "Nuke Review Bombing (Coordinated 1-Star on Head)",
...
st.sidebar.markdown("---")
st.sidebar.subheader("RRFN & Detector Hyperparameters")
...
if llm_provider == "gemini":
    ...
    if not gemini_api_key_input:
        st.sidebar.warning("No key entered; falling back to mock auditor.")
...
# =============================================================================
with tab1:
    st.subheader("1. Multi-View Signal Fusion & Weight Distribution")
...
# =============================================================================
with tab2:
    st.subheader("2. Statistical Noise Transition Modeling & Denoising Diagnostics")
...
# =============================================================================
with tab3:
    st.subheader("3. Base Model vs Updated Model: Head-to-Head Benchmark")
    st.markdown("""
...
# =============================================================================
with tab5:
    st.subheader("5. Live LLM Semantic Auditor & Prompt Walkthrough")
    st.markdown("""
    ...
    if audit_records:
        ...
        if filtered_records:
            rec_indices = [f"User #{r['user_id']} → {r['title'][:25]} ({r['rating']}/5, Score={r['score']:.2f})" for r in filtered_records]
...
# =============================================================================
with tab6:
    st.subheader("6. Individual User Profile & Top-10 What-If Sandbox")
    st.markdown("""
    ...
    st.subheader("Interactive 'What-If' Inclination Sandbox (Real-Time Client Merge)")
