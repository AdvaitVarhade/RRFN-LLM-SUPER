...
| H6 (Tri-Tier Partition) | 6 | 0.024 | 0.055 | 0.018 | -20% recall |
| H8 (Logit Calibration) | 8 | 0.024 | 0.150 | 0.019 | tau=0.5 |
...
## Extended Architectural Findings (H6, H7, H8)

1. **Tri-Tier Partitioning (H6) Exacerbates Sparsity**: Splitting the catalog into Head, Torso, and Tail (3 pools) dropped Recall from 0.030 to 0.024. In a federated setting, the Torso and Tail models receive so few interactions per client that they struggle to learn meaningful representations. The binary Pareto split is optimal because it preserves enough collaborative signal in the Tail pool.
2. **Shared-Backbone Negative Transfer (H7)**: Training a single global backbone and projecting it into Head/Tail heads also harmed accuracy (Recall dropped to 0.023). Because the Head items dominate the global loss landscape, the shared embeddings align entirely to Head item preferences. When the Tail head projects this backbone, the representations are too skewed to rank tail items effectively. Completely isolating the models (the original SUPER approach) prevents this negative transfer.
3. **Continuous Logit Adjustment Fails (H8)**: Replacing hard integer quotas with continuous logit-adjustment ($\tau \cdot (1 - \text{Pop}_u) \cdot \log P_i$) causes a catastrophic accuracy collapse. At $\tau = 0.5$, Recall drops to 0.024 and Rmse-PC only reaches 0.150 (far worse than FedSUPER's 0.055). At $\tau = 1.0$, Recall drops to a near-zero 0.003. Continuous penalties destroy the subtle collaborative filtering ranking signals. The hard-quota blueprint merge is mathematically superior because it preserves pure, unpenalized intra-pool ranking while guaranteeing global calibration via hard structural interleaving.