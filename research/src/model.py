...
class SharedBackboneRecMF(nn.Module):
    """
    Hypothesis 7 (H7): Shared-Backbone Federated Transfer Learning
    Trains a shared user and item backbone on ALL interactions.
    Applies separate projection heads for Head vs. Tail (and optionally Torso) 
    to specialize scoring while preventing cold-start sparsity in the tail.
    """
    def __init__(self, n_users, n_items, dim=64, seed=0, num_heads=2):
        super().__init__()
        torch.manual_seed(seed)
        self.dim = dim
        self.num_heads = num_heads  # e.g., 2 for (Head, Tail) or 3 for (Head, Torso, Tail)
        
        # Shared representations
        self.user_backbone = nn.Embedding(n_users, dim)
        self.item_backbone = nn.Embedding(n_items, dim)
        
        # Pool-specific linear projections for items
        # Transforms the shared item backbone into pool-specific semantic spaces
        self.head_projections = nn.ModuleList([
            nn.Linear(dim, dim, bias=False) for _ in range(num_heads)
        ])
        
        self.item_biases = nn.ModuleList([
            nn.Embedding(n_items, 1) for _ in range(num_heads)
        ])
        
        # Init
        self.user_backbone.weight.data.uniform_(-0.01, 0.01)
        self.item_backbone.weight.data.uniform_(-0.01, 0.01)
        for proj in self.head_projections:
            # Initialize close to identity matrix for stable transfer learning
            proj.weight.data = torch.eye(dim) + torch.randn(dim, dim) * 0.01
        for bias in self.item_biases:
            bias.weight.data.zero_()

    def forward(self, u, i, head_idx=0):
        """
        head_idx: 0 for Head pool, 1 for Tail pool (or Torso, etc.)
        """
        eu = self.user_backbone(u)
        ei_shared = self.item_backbone(i)
        
        # Apply pool-specific projection
        ei_proj = self.head_projections[head_idx](ei_shared)
        
        score = (eu * ei_proj).sum(dim=1) + self.item_biases[head_idx](i).squeeze(1)
        return score, eu, ei_proj

    def score_matrix(self, head_idx=0):
        """Return full (n_users x n_items) score matrix for a specific projection head."""
        with torch.no_grad():
            U = self.user_backbone.weight
            I_shared = self.item_backbone.weight
            I_proj = self.head_projections[head_idx](I_shared)
            B = self.item_biases[head_idx].weight.squeeze(1)
            S = (U @ I_proj.T) + B[None, :]
        return S.detach().cpu().numpy().astype(np.float32)
